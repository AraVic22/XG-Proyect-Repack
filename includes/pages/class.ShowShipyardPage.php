...
class ShowShipyardPage
{
	private function GetMaxConstructibleElements ($Element, $Ressources)
	{
		...
		if (isset($pricelist[$Element]['metal']) and $pricelist[$Element]['metal'] > 0)
			$MaxElements        = min($MaxElements, floor($Ressources["metal"] / $pricelist[$Element]['metal']));

		if (isset($pricelist[$Element]['crystal']) and $pricelist[$Element]['crystal'] > 0)
			$MaxElements        = min($MaxElements, floor($Ressources["crystal"] / $pricelist[$Element]['crystal']));

		if (isset($pricelist[$Element]['deuterium']) and $pricelist[$Element]['deuterium'] > 0)
			$MaxElements        = min($MaxElements, floor($Ressources["deuterium"] / $pricelist[$Element]['deuterium']));
			
		if (isset($pricelist[$Element]['tritium']) and $pricelist[$Element]['tritium'] > 0)
			$MaxElements        = min($MaxElements, floor($Ressources["tritium"] / $pricelist[$Element]['tritium']));

		if (isset($pricelist[$Element]['energy']) and $pricelist[$Element]['energy'] > 0)
	...
	private function GetElementRessources($Element, $Count)
	{
		...
		$ResType['tritium']   = ($pricelist[$Element]['tritium']   * $Count);
				...
				if ($Count != 0)
				{
					if ( IsTechnologieAccessible ($CurrentUser, $CurrentPlanet, $Element) )
					{
						...
						if ($Count >= 1)
						{
							...
							$CurrentPlanet['crystal']        -= $Ressource['crystal'];
							$CurrentPlanet['deuterium']      -= $Ressource['deuterium'];
							$CurrentPlanet['tritium']        -= $Ressource['tritium'];
